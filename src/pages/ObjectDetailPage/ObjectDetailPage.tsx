...
import { TabContent, LoadingWrapper, NotFoundText } from './ObjectDetailPage.styles';
...
const ObjectDetailPage: React.FC = () => {
  ...
  return (
    <>
      <ObjectHeader object={object} statusLabels={statusLabels} />

      <Tabs tabs={TABS} activeTab={tab} onChange={(t) => setTab(t as TabId)} />

      <TabContent>
        {tab === 'notes' && (
          <NotesFeed objectId={object.id} objectTitle={object.title} highlightNoteId={noteParam ?? undefined} />
        )}

        {tab === 'photos' && (
          <PhotoGrid objectId={object.id} highlightPhotoId={photoParam ?? undefined} objectTitle={object.title} />
        )}
